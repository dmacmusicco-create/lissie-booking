...
function formatSubjectGroups(groups: string[][]): string {
  ...
  return groups.map(group => {
    if (group.length === 1) return new Date(group[0] + 'T12:00:00').toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
...
export async function sendClientConfirmation(booking: BookingRequest): Promise<void> {
  ...
  await resend.emails.send({
    ...
    html: '<p>Dear ' + booking.name + ',</p>'
      ...
      + (booking.fileLink && booking.fileLink.trim() !== '' && booking.attachments && booking.attachments.length > 0 ? '<p>Your file link and attachment(s) have been received.</p>' : booking.fileLink && booking.fileLink.trim() !== '' ? '<p>Your file link has been received.</p>' : booking.attachments && booking.attachments.length > 0 ? '<p>Your attachment(s) have been received.</p>' : '')
